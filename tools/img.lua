...
local sizes = {360, 720, 1024}
local src_transformation = '?tr=w-360'
local property = 'Gulf Place Residences'
local path = 'images'
-- local path = 'images/prominence'
-- local property = 'Prominence North'
...
for _, image in ipairs(images) do
  ...
  for _, n in ipairs(sizes) do
    ...
    local src = string.format("%s%s", url, transformation)
    table.insert(srcset_paths, src)
  end
  local srcset = table.concat(srcset_paths, ",\n                ")
  local src = table.concat({domain, id, path, image}, '/') .. src_transformation
